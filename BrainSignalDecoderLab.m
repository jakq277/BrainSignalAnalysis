...
imagesc([R(20).spikeRaster; R(20).spikeRaster2]);
xlabel('Time [ms]');
...
for i = 1:size(Y_bin, 1) - 1
    Y_bin_smooth(i,:) = conv(Y_bin(i,:), [1 1 1 1 1], 'same');
...
for i = 1:numel(Rtest)
    ...
    for j = 1:size(Ytest, 1)
        Ytest(j,:) = conv(Ytest(j,:), [1 1 1 1 1], 'same');
    end
    Ytest = [Ytest; ones(1, size(Ytest, 2))]; % append 1's for the bias term
...
Y_bin_dif = Y_bin(:,2:end) - Y_bin(:,1:end-1);
X_bin_origin = X_bin(:,:);
X_bin = X_bin(:,1:end - 1);

%%
% this line below builds the decoder
L = X_bin * pinv(Y_bin_dif);
...
for i = 1:numel(Rtest)
    ...
    Ytest = [Rtest(i).spikeRaster; Rtest(i).spikeRaster2]; % extract test data
    Ytest = bin(Ytest, dt, 'sum'); % bin the data
    Ytest = Ytest(:,2:end) - Ytest(:,1:end-1);
...
Y_bin_kernel = zeros(size(Y_bin));

for i = 1:size(Y_bin, 1) - 1
    Y_bin_kernel(i,:) = conv(Y_bin(i,:), [-1 -1 4 -1 -1], 'same');
...
%%
% this line below builds the decoder
L = X_bin_origin * pinv(Y_bin_kernel);
...
for i = 1:numel(Rtest)
    ...
    for j = 1:size(Ytest, 1)
        Ytest(j,:) = conv(Ytest(j,:), [-1 -1 4 -1 -1], 'same');
    end
    Ytest = 1 - Ytest;
    Ytest = [Ytest; ones(1, size(Ytest, 2))]; % append 1's for the bias term
